...
template <int gemm_k, int tile_m, int tile_n, int tile_k, int stage_cnt>
struct GmemLoaderA
{
...
struct MmaComputer
{
    ...
    __device__ MmaComputer(
        bf16_t* gmem_c_local_, bf16_t* smem_a_, bf16_t* smem_b_, uint64_t* smem_barrier_, int warp_idx_)
...
__global__ __launch_bounds__(256, 1) void super_mma_gemm_per_warp_per_cta(
    bf16_t* output, bf16_t const* mat_a, bf16_t const* mat_b)
{
    ...
    if (warp_idx < 2)
    {
        GmemLoaderA<gemm_k, tile_m, tile_n, tile_k, stage_cnt> a_loader(gmem_a_local, smem_a, smem_barrier);
        a_loader.prepare();
        a_loader.issue_mainloop();
    }
    else if (warp_idx < 4)
    {
        GmemLoaderB<gemm_k, tile_m, tile_n, tile_k, stage_cnt> b_loader(gmem_b_local, smem_b, smem_barrier);
        b_loader.prepare();
    ...
    else
    {
        MmaComputer<gemm_m, gemm_n, gemm_k, tile_m, tile_n, tile_k, stage_cnt> mma_computer(
            gmem_c_local, smem_a, smem_b, smem_barrier, warp_idx);
...
void invokeFuseAGemm(T* output, T const* mat_a, T const* mat_b, cudaStream_t const stream)
{
    ...
    int cta_m_cnt = gemm_m / tile_m;                               // 2112 / 16 = 132
    int cta_n_cnt = gemm_n / tile_n;                               // 4
    ...
    if (smem_bytes >= (48 * 1024))
    {
        TLLM_CUDA_CHECK(cudaFuncSetAttribute(
            super_mma_gemm_per_warp_per_cta<batch_size, gemm_m, gemm_n, gemm_k, tile_m, tile_n, tile_k, stage_cnt>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes));
    }
    TLLM_CUDA_CHECK(cudaLaunchKernelEx(&config,
        super_mma_gemm_per_warp_per_cta<batch_size, gemm_m, gemm_n, gemm_k, tile_m, tile_n, tile_k, stage_cnt>, output,
        mat_a, mat_b));
}
