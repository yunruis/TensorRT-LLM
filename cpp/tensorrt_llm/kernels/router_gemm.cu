...
__global__ __launch_bounds__(128, 1) void router_gemm_kernel(float* out, T const* mat_a, T const* mat_b)
{
    ...
    // Initialize accumulators for all M rows (in pairs)
    float2 acc[kNumTokens / 2] = {};
...
#pragma unroll
        for (int m_pair = 0; m_pair < kNumTokens / 2; m_pair++)
        {
            int const m_idx = m_pair * 2;

            // Load both rows of A matrix using vector loads
            uint4 a_vec0 = *reinterpret_cast<uint4 const*>(mat_a + (m_idx * kHiddenDim) + k_base);
            uint4 a_vec1 = *reinterpret_cast<uint4 const*>(mat_a + ((m_idx + 1) * kHiddenDim) + k_base);

            // Convert A values to float
            float a_float0[VPT];
            float a_float1[VPT];
            bf16_uint4_to_float8<VPT>(a_vec0, a_float0);
            bf16_uint4_to_float8<VPT>(a_vec1, a_float1);

// Process elements in this chunk
#pragma unroll
            for (int k = 0; k < VPT; k++)
            {
                float2 a_pair = make_float2(a_float0[k], a_float1[k]);
                float2 b_pair = make_float2(b_float[k], b_float[k]);
                fma(acc[m_pair], a_pair, b_pair, acc[m_pair]);
            }
        }
    }
...
#pragma unroll
    for (int m_pair = 0; m_pair < kNumTokens / 2; m_pair++)
    {
        warp_result[m_pair * 2] = acc[m_pair].x;
        warp_result[m_pair * 2 + 1] = acc[m_pair].y;
    }
